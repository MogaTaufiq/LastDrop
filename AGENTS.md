...
| `scene-task3` | `'task3'` | Stop oil leak — drag bolts from the toolbox onto the patch over the crack (4 bolts → seal) |
...
|---|---|---|
| 2026-06-09 | **Advisor (dosen pembimbing) revision round — 17 items.** (1) **i18n fix:** the `TRANSLATIONS.en` dictionary was empty, so short keys (`crisis_body`, `tap_to_start`, `btn_*`, …) rendered as raw key text when returning to English; populated `en` with all 45 short keys. (2) **Filtration soft-lock fix (`initFiltrationGame`):** `.hidden{display:none !important}` blocked the inline `display:flex` reveal of layers 2–3, so the task never completed; reveal now uses `classList.remove('hidden')` + per-entry layer reset for clean re-entry. (3) **Opening images:** slides 8/9 now point to the provided `scene_8.webp`/`scene_9.webp` (dramatic stranded whale + diseased-water visuals). (4) **Global per-scene Back button** (`#btn-back`, `goBack`/`updateBackButton`/`prevSlide`/`exitCurrentAreaToResidential`): prev-slide in the cinematic, confirm-and-exit to Residential inside tasks (resets coastal `tasksCompleted`), hidden on landing/residential/reflection. (5) **Crisis gating:** the first crisis now appears only after the player explores ≥3 distinct residential objects **or** a 30s fallback (`beginExplorationPhase`, `recordExploration`, `doResInteract`), replacing the immediate post-modal trigger; added a residential explore-hint banner (`#res-explore-hint`). (6) **Explore-map avatar** swapped to the detailed/brighter residential character + a "YOU" marker and pulse ring; clearer "move to Coastal & press E" guidance. (7) **Stat-gain popups** (`showStatGain`): prominent animated 💧/🦋 chips on every task completion. (8) **Coastal Task 2:** trash repositioned out of the bottom-right checklist zone. (9) **Coastal Task 3** reworked glue→**bolt-the-patch** (toolbox bolts dragged onto a patch over the leak; `metal_drag`/`wrench_ratchet` SFX) — mirrors the discharge fix. (10) **Task 4:** brighter ocean gradient, enlarged/glowing Corexit boat, and removed the `Recommended`/`May harm`/stat-preview hints from the choice cards (also Industrial Task 3 cards) so students decide unaided. (11) **Agri Task 1/2:** clearer click guidance + visible pulsing "click to scan" / "+" plant-spot affordances. (12) **Industrial Task 1:** rebuilt the flat gray scene into a factory (sawtooth roof, lit windows, two smokestacks with animated smoke, discharge pipe) + clear click target. (13) **Industrial Task 2:** rebuilt the floating bolts into a real **pipe-flange joint** (`#ind2-joint` HTML assembly) so bolts visibly sit on the flange; clearer instruction. Added EN+ID translations for all new copy. Verified end-to-end via local preview (no console errors). **Follow-ups same day:** Back button moved to bottom-left and restyled to match the cinematic Next button (rounded rectangle, level pair); added a dark scrim behind `.task-header` so titles/instructions stay legible on bright backgrounds (Agri/beach/ocean); **fixed an audio bug where background loops could stack** (`Audio.play` now tracks `bgMusic` synchronously and pauses the previous loop before starting a new one, so factory ambience can no longer bleed into ocean scenes) and made `initResidential` play `ambient_ocean` for the base world; added `?v=` cache-busting to the `css/style.css` and `js/game.js` includes so players don't get stale cached code after updates. | Claude |
| 2026-05-29 | **Explore Map Scene & Minimap Restoration**: Restored the missing `#mini-map-container` in `index.html`. Converted the `#explore-map-overlay` full screen fixed overlay to a proper fullscreen scene `#scene-explore-map` managed by `SceneManager`. Refactored `game.js` to transition between `scene-residential`, `scene-explore-map`, and task scenes without flashing the residential base. Moved the mobile virtual joystick to a global scope and added CSS `:has()` selector to show it conditionally in active play scenes on mobile coarse pointer devices. | Antigravity |
| 2026-05-29 | **Custom Opening Cinematic Backgrounds:** Configured opening cinematic system slides to load images from `assets/images/opening/` as background elements, mapping `scene_1.jpg` through `scene_10_11.jpg` sequentially. Renamed typo file paths (`scence_8.jpg` -> `scene_8.jpg`, `scence_9.jpg` -> `scene_9.jpg`) to align with conventions and modified `renderSlide` in `js/game.js` to render the `<img>` element instead of the inline SVG placeholders. | Antigravity |
